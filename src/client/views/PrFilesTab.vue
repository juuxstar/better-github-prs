<template>
  <div
    class="pr-files-tab u-flex u-flex-col u-flex-1 u-min-h-0 u-overflow-hidden"
    ...
    <template v-else-if="files.length">
      ...
      <div v-else-if="hasFullContent" class="pr-diff-viewer" :class="{ 'pr-diff-viewer-split' : !isAddedOrRemoved, 'pr-diff-viewer-with-minimap' : isAddedOrRemoved }">
        <template v-if="isAddedOrRemoved">
          <div
            class="pr-diff-panel-full u-flex-grow-1 u-min-w-0"
            :class="{ 'pr-diff-panel-full-del' : currentFile.status === 'removed' }"
          >
            <pr-diff-table

...
export function getStoredHljsThemeId(scheme: ResolvedScheme): string {
  ...
  if (raw && list.some(t => t.id === raw)) return raw;
...
export function migrateHljsThemeStorage(): void {
  ...
  if (meta?.appearance === 'light') {
    if (!hasL) localStorage.setItem(KEY_LIGHT, legacy);
    if (!hasD) localStorage.setItem(KEY_DARK, defaultHljsThemeId('dark'));
  } else {
    const darkId = meta && themeModules[legacy] ? legacy : defaultHljsThemeId('dark');
    if (!hasD) localStorage.setItem(KEY_DARK, darkId);
    if (!hasL) localStorage.setItem(KEY_LIGHT, defaultHljsThemeId('light'));
  }
  try {

...
export function computeInlineHighlights(
  ...
  if (n === 0 && m === 0) return { oldRanges: [], newRanges: [] };
  if (n === 0) return { oldRanges: [], newRanges: [{ start: 0, end: newText.length }] };
  if (m === 0) return { oldRanges: [{ start: 0, end: oldText.length }], newRanges: [] };
  ...
  const oldRanges = mergeConsecutiveRanges(
    edits
      .filter(e => e.type === 'del')
      .map(e => ({ start: oldOffsets[e.oldIdx!], end: oldOffsets[e.oldIdx!] + oldToks[e.oldIdx!].length })),
  );
  const newRanges = mergeConsecutiveRanges(
    edits
      .filter(e => e.type === 'add')
      .map(e => ({ start: newOffsets[e.newIdx!], end: newOffsets[e.newIdx!] + newToks[e.newIdx!].length })),
  );
...
export function applyHighlightMarks(
  ...
  function insideMarkNeeded(): boolean {
    if (rangeIdx >= ranges.length) return false;
 ...
 */
export function annotateLinePair(
  delLine: { content: string; html?: string },
  addLine: { content: string; html?: string },
): void {

...
export function applyWordDiffToFullContent(left: DiffLine[], right: DiffLine[], patch: string | undefined) {
  ...
  for (const l of left) if (l.num != null) leftByNum.set(l.num, l);
  const rightByNum = new Map<number, DiffLine>();
  for (const r of right) if (r.num != null) rightByNum.set(r.num, r);
  ...
  for (const hunk of hunks) {
    ...
    while (i < hunk.lines.length) {
      ...
      for (let p = 0; p < pairCount; p++) {
        ...
        if (delLine && addLine) annotateLinePair(delLine, addLine);
      }
...
export function applyWordDiffToPatchLines(left: DiffLine[], right: DiffLine[]) {
  ...
  while (li < left.length && ri < right.length) {
    const dels: DiffLine[] = [];
    while (li < left.length && left[li].type === 'del') dels.push(left[li++]);
    const adds: DiffLine[] = [];
    while (ri < right.length && right[ri].type === 'add') adds.push(right[ri++]);
    ...
    while (li < left.length && left[li].type !== 'del') li++;
    while (ri < right.length && right[ri].type !== 'add') ri++;
  }
}
...
export function parsePatch(patch: string | undefined, file: PRFile | null): { left: DiffLine[]; right: DiffLine[] } {
  ...
  if (file) {
    ...
    if (leftHtml) {
      let hi = 0;
      for (const l of left) {
        if (l.type !== 'hunk') l.html = leftHtml[hi++];
      }
    }
    if (rightHtml) {
      let hi = 0;
      for (const l of right) {
        if (l.type !== 'hunk') l.html = rightHtml[hi++];
      }
    }
  }
...
export function buildSplitLinesForFile(
  file: PRFile,

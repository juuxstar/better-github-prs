class GitHubAPI {
  ...
  async fetchBranchesWithoutPRs(repo: string): Promise<BranchInfo[]> {
    const [branches, openPRs] = await Promise.all([
      ...
      this.fetchAllPages(`/repos/${repo}/pulls?state=open`),
    ]);
    const prHeads = new Set(
      openPRs
        .filter((pr: any) => pr.head?.repo?.full_name?.toLowerCase() === repo.toLowerCase())
  ...
  async fetchPullRequestReviewDecision(
    owner: string,
    repo: string,
    number: number,
    ...
    try {
      ...
      if (rd === 'APPROVED' || rd === 'CHANGES_REQUESTED' || rd === 'REVIEW_REQUIRED') return rd;
      return null;
    } catch {
      return null;
    }
  ...
  private async fetchReviewCommentThreadMeta(
    owner: string,
    repo: string,
    number: number,
  ): Promise<Map<number, { isResolved: boolean; threadNodeId: string }>> {

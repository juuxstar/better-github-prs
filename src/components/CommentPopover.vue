      ...
      <div v-for="c in thread.comments" :key="c.id" class="cp-comment">
        ...
        <div class="cp-body">
          <template v-for="(seg, si) in parseBody(c.body)" :key="si">
            <p v-if="seg.type === 'text'" class="cp-text">{{ seg.content }}</p>
...
.cp-type-change-required { background: rgba(210, 153, 34, 0.15); color: var(--accent-orange); }
.cp-type-question { background: rgba(88, 166, 255, 0.12); color: var(--accent-blue); }
...
.cp-type-pill {
  ...
  &.active { border-color: var(--accent-blue); color: var(--accent-blue); background: rgba(88, 166, 255, 0.08); }
}
...
.cp-btn-danger {
  ...
  &:hover:not(:disabled) { background: rgba(248, 81, 73, 0.25); }
}
</style>
